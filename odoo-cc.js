...
// Resultados: sobrantes / faltantes por sucursal (desde cc_revision)
async function resultados(p) {
  const tareaId = p.tarea_id;
  const rev = await sbSelectAll(`cc_revision?tarea_id=eq.${tareaId}&select=location_id,product_id,estado,contado,teorico&order=id.asc`);
  const sucs = await sbSelectAll(`cc_tarea_sucursal?tarea_id=eq.${tareaId}&select=location_id,location_name&order=location_name.asc`);
  const nameMap = {}; sucs.forEach((s) => (nameMap[s.location_id] = s.location_name));
  const byLoc = {};
  rev.forEach((r) => {
    const d = Number(r.contado || 0) - Number(r.teorico || 0);
    const L = byLoc[r.location_id] || (byLoc[r.location_id] = {
      location_id: r.location_id, location_name: nameMap[r.location_id] || ("Ubic " + r.location_id),
      sobrantes: 0, faltantes: 0, lineas: 0, dif_lineas: 0, aprobadas: 0, rechazadas: 0,
    });
    L.lineas++;
    if (r.estado === "aprobada") L.aprobadas++;
    if (r.estado === "rechazada") L.rechazadas++;
    if (d > 0) L.sobrantes += d; else if (d < 0) L.faltantes += -d;
    if (d !== 0) L.dif_lineas++;
  });
  const sucursales = Object.values(byLoc).map((L) => ({ ...L, neto: L.sobrantes - L.faltantes }));
  sucursales.sort((a, b) => String(a.location_name).localeCompare(String(b.location_name)));
  const total = sucursales.reduce((a, s) => { a.sobrantes += s.sobrantes; a.faltantes += s.faltantes; a.dif_lineas += s.dif_lineas; a.lineas += s.lineas; return a; }, { sobrantes: 0, faltantes: 0, dif_lineas: 0, lineas: 0 });
  total.neto = total.sobrantes - total.faltantes;
  return { sucursales, total, revisadas: rev.length };
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case "resultados": result = await resultados(payload); break;
